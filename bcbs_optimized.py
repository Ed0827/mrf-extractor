...
- NEW: --one-file-per-code keeps a single CSV per CPT (no .partN), faster and cleaner
- Uploads to R2 at end (or immediately in legacy per-item mode)

Usage:
  python3 bcbs_extract.py <input.json.gz> <output_dir> <r2_prefix>
                          [--chunk 500000] [--ephemeral]
                          [--pigz-threads 0]
...
def parse_args():
    ...
    p.add_argument("--ephemeral", action="store_true",
                   help="(Legacy) Upload each CPT CSV when an item ends (slower; many small files)")
    ...
    p.add_argument("--one-file-per-code", action="store_true",
                   help="Keep a single CSV open per CPT for the entire run (much faster, no .partN)")
    return p.parse_args()
...
def main():
    ...
    # Decompress via pigz if available for speed
    use_proc = False
    fh = None
    try:
        ...
        for item in ijson.items(fh, "in_network.item"):
            ...
            for rate in rates:
                ...
                # Stream rows directly (no intermediate 'prepped' list)
                for pg in pgs:
                    tin = pg.get("tin") or {}
