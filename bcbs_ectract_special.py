...
def parse_args():
    ...
    p.add_argument("--codes", nargs="+", default=None,
                   help="Only extract these CPT codes (space- or comma-separated). "
...
def main():
    ...
    # NEW: normalize allowlist into a set of strings
    allowed_codes = None
    if args.codes:
        allowed_codes = set()
        for tok in args.codes:
            # support comma- or space-separated tokens
            for c in tok.replace(",", " ").split():
                if c:
                    allowed_codes.add(str(c))
    ...
    try:
        ...
        for item in ijson.items(fh, "in_network.item"):
            ...
            # NEW: skip CPTs not in allowlist (if provided)
